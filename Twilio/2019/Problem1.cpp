...
public:
    ...
    void insert(string word) {
        Trie*cur = this;
        rep(i, sz(word)) {
            if(!cur->link[word[i]-'0']) {
                cur->link[word[i]-'0']=new Trie();
            }
            cur = cur->link[word[i]-'0'];
        }
        cur->isEnd=true;
        cur->uler=word;
    }
...
//         Trie* cur = this;
//         rep(i, sz(word)) {
//             if(!cur->link[word[i]-'0']) cur->link[word[i]-'0']=new Trie();
//             cur = cur->link[word[i]-'0']; 
//         }
//         cur->uler = word;
//         cur->isEnd=true;
...
vector<string> solve(vector<string> &areaCode, vector<string> &phoneNums) {
    Trie* root = new Trie();
    for(auto &c: areaCode) {
        root->insert(c);
    }
    vector<string> res;
    for(auto &s: phoneNums) {
        string lis = "";
        rep(i, sz(s)) {
            Trie* cur = root->link[s[i]-'0'];
            if(!cur)break;
            rng(j, i+1, sz(s)) {
                if(cur->isEnd && cur->uler.size() > sz(lis)) {
                    lis=cur->uler;
                }
                if(cur->link[s[j]-'0']) cur=cur->link[s[j]-'0'];
                else break;
            }
        }
        res.pb(lis);
...
int main() {
    ...
    vector<string> a = {"213", "21358", "1234", "12"};
    vector<string> b = {"21349049", "1204539492", "123490485904"};
    vector<string> lis = solve(a, b);
    for(auto &x: lis) 
        cout << x << sp;
